...
async function deployContract(contract_title: string, args?: any) {
    ...
    var dapp;
    if (args == null) {
        dapp = await Contract.deploy(tx_params);
    } else if (args.length == 1) {
        dapp = await Contract.deploy(args[0], tx_params);
    } else if (args.length == 2) {
        dapp = await Contract.deploy(args[0], args[1], tx_params);
...
function save_artifacts(contract_name: string, dapp: any, chainId: number) {
    // Save deployed smart contract artifacts to dapp folder
    // Gets artifacts from contract
    const dapp_artifact = hre.artifacts.readArtifactSync(contract_name);
